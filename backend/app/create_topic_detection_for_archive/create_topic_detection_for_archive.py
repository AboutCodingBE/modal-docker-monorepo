...
class CreateTopicDetectionForArchive:
    ...
    async def execute(
        ...
        try:
            ...
            # Nabewerking: tag_index vullen vanuit de TopicDetection-rijen die we net
            # hebben weggeschreven. Draait ná de COMPLETED-status hierboven — fouten
            # hier worden intern door CreateTagIndexForArchive gelogd, niet doorgegooid,
            # en wijzigen dus de net voltooide topic-detection-status niet.
            await CreateTagIndexForArchive(self._session_factory).execute(
                archive_id, archive_analysis_id, source="topic_detection"
            )
