...
class FileRepository:
    ...
    async def persist_all(self, entries: list[dict]) -> None:
        path_to_id: dict[str, uuid.UUID] = {}
        for batch_start in range(0, len(entries), BATCH_SIZE):
            batch = entries[batch_start: batch_start + BATCH_SIZE]
            await self._persist_batch(batch, path_to_id)

    async def _persist_batch(self, batch: list[dict], path_to_id: dict[str, uuid.UUID]) -> None:
        for entry in batch:
