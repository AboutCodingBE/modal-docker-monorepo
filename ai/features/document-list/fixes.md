...
## 1. Sort doesn't cover all four requested fields

Source spec: sort by datum, pad, documenttype, klasse. Current `_VALID_SORT_FIELDS`:

```python
_VALID_SORT_FIELDS = {"content_created_at", "relative_path", "category"}
```

`mime_type` (documenttype) is missing — only `category` (klasse) is sortable, not document type.
Needs a fourth branch in the `if sort_by == ...` chain in `list_files()`, sorting on
`TikaAnalysis.mime_type`, same `nullslast(direction(...))` pattern as `category` (mime_type is
nullable, same as the other Tika-sourced fields).
